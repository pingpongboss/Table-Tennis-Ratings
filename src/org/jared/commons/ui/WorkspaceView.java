...
public class WorkspaceView extends ViewGroup {
	...
	private OnScrollToScreenListener mScrollListener;
...
//		}
	...
	public void setCurrentScreen(int theCurrentScreen) {
		...
		currentScreen = Math
				.max(0, Math.min(theCurrentScreen, getChildCount()));
...
//	private int lastEvHashCode;
	...
	private void updateTabIndicator() {
		...
		int bitmapHeight = (TAB_INDICATOR_HEIGHT_PCT * height / 100);
		bitmap = Bitmap.createBitmap(width == 0 ? 1 : width,
				bitmapHeight == 0 ? 1 : bitmapHeight, Bitmap.Config.ARGB_8888);
	...
	public void setOnScrollToScreenListener(
			OnScrollToScreenListener scrollListener) {
	...
	private void scrollToScreen(int whichScreen, boolean immediate) {
		...
		scroller.startScroll(getScrollX(), 0, delta, 0,
				immediate ? 0 : Math.abs(delta) * 2);
		invalidate();
		mScrollListener.OnScrollToScreen(whichScreen);
	}
